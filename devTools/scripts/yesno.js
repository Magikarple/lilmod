'use strict';

// Copied from https://github.com/tcql/node-yesno/blob/master/yesno.js
// as to not have it as a node dependency
// reformated to pass our ESLint linting

// const readline = require('readline');
// @ts-ignore
import readline from "readline";


const options = {
	yes: [ 'yes', 'y' ],
	no:  [ 'no', 'n' ]
};


function defaultInvalidHandler({question, defaultValue, yesValues, noValues}) {
	let yValues = (yesValues || options.yes);
	let nValues  = (noValues || options.no);

	process.stdout.write('\nInvalid Response.\n');
	process.stdout.write('Answer either yes : (' + yValues.join(', ') + ') \n');
	process.stdout.write('Or no: (' + nValues.join(', ') + ') \n\n');
}


async function ask({question, defaultValue, yesValues, noValues, invalid}) {
	if (!invalid || typeof invalid !== 'function') { invalid = defaultInvalidHandler; }

	let yValues = (yesValues || options.yes).map((v) => v.toLowerCase());
	let nValues  = (noValues || options.no).map((v) => v.toLowerCase());

	const rl = readline.createInterface({
		input: process.stdin,
		output: process.stdout
	});

	return new Promise(function(resolve, reject) {
		rl.question(question + ' ', async function(answer) {
			rl.close();

			const cleaned = answer.trim().toLowerCase();

			if (cleaned === '' && defaultValue != null) { return resolve(defaultValue); }

			if (yValues.indexOf(cleaned) >= 0) { return resolve(true); }

			if (nValues.indexOf(cleaned) >= 0) { return resolve(false); }

			invalid({
				question, defaultValue, yesValues, noValues
			});
			const result = await ask({
				question, defaultValue, yesValues, noValues, invalid
			});
			resolve(result);
		});
	});
}


// module.exports = ask;
export {ask};
