App.Events.recFSGenderRadicalist = class recFSGenderRadicalist extends App.Events.BaseEvent {
	eventPrerequisites() {
		return [
			() => V.FSAnnounced === 1,
		];
	...
	get weight() {
		return V.arcologies[0].FSGenderRadicalist > random(1, 100) ? 1 : 0;
	}
	...
	execute(node) {
		...
		r = [];
		r.push(`This one is from a rather abused individual from one of the more traditionalist areas of the old world. It seems ${he}'s heard of the new and interesting way gender is thought of in the Free Cities, and in ${V.arcologies[0].name} in particular.`);
		r.push(Spoken(slave, `"Being your slave has to be better than being a subhuman here,"`));
		r.push(`${he} says.`);
		...
		function sell() {
			...
			return `When ${he} arrives, ${he} comes directly to your penthouse for enslavement. ${He} wears an expression of doubt, fear, and wonder as ${he} takes in the sights and sounds of the magnificent beast that is the new society taking shape in ${V.arcologies[0].name}. A purchaser's agent arrives to take charge of ${him}, and ${he} asks politely who bought ${him}. A salon, you tell ${him}; ${he}'ll be trained to ${he} tables, serve drinks, and entertain customers with ${his} mouth and anus.`;
		}
	}
};
